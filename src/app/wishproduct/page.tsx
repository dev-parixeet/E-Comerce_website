'use client'
import { getwishlist } from '@/stores/dataStore';
import React, { useEffect } from 'react'

type Props = {}

const wishproduct = (props: Props) => {
    const { getdata, getApiData }: any = getwishlist();
    console.log(getdata, "getproduct page wishlist")
    useEffect(() => {
        getApiData();
    }, []);
    ...
    return (
        <>
            <div className="h-screen mt-24 p-20">
                {getdata?.map((wishlishtproduct:any,index:number) => {
                    return <div className="flex font-sans">
                        <div key={index}
                        className="flex-none w-56 relative">
                            <img
                                src={wishlishtproduct?.image}
                                alt=""
                                className="absolute inset-0 w-full h-80 object-cover rounded-lg"
                                loading="lazy"
                            />
                        </div>
                        <form className="flex-auto p-6">
                            <div className="flex flex-wrap">
                                ...
                                <div className="flex items-baseline mt-4 mb-6 pb-6 border-b border-violet-400"></div>
                                <div className="w-full flex-none mt-2 order-1 text-3xl font-bold text-violet-600">
                                   $ {wishlishtproduct?.price}
                                </div>
                                <div className=" flex text-sm font-medium text-white bg-violet-500 rounded-full w-28 justify-center items-center h-10">In stock</div>
                            </div>
                            <div className="flex items-baseline mt-4 mb-6 pb-6 border-b border-violet-400">
